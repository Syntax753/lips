...
import { solve, bestMove, planToLURD } from "./solve.js";
...
test("planToLURD: a single push is one UPPERCASE move that replays to a win", () => {
  // '@$.' — push the box one cell right onto the goal: no walking, one push.
  const r = solve("@$.");
  const m = planToLURD("@$.", r.plan!);
  assert.equal(m.valid, true);
  assert.equal(m.lurd, "R");
  assert.equal(m.moves, 1);
  assert.equal(m.pushes, 1);
});

test("planToLURD: walking steps are lowercase and the count matches the solve", () => {
  // '@ x' — the player just walks two cells right onto the goal, no pushes.
  const r = solve("@ x");
  const m = planToLURD("@ x", r.plan!);
  assert.equal(m.valid, true);
  assert.equal(m.lurd, "rr");
  assert.equal(m.pushes, 0);
  assert.equal(m.moves, r.moves); // optimal solve → replayed length is the minimum
});

test("planToLURD: mixes a push and walks, one char per player step", () => {
  // Push the box down (one UPPERCASE), then walk to the player goal — 4 moves.
  const grid = "#@##\n#$ #\n#  #\n# x#";
  const r = solve(grid);
  const m = planToLURD(grid, r.plan!);
  assert.equal(m.valid, true);
  assert.equal(m.moves, r.moves);
  assert.equal(m.pushes, 1);
  assert.equal(m.lurd.length, m.moves); // one character per player step
  assert.equal((m.lurd.match(/[A-Z]/g) ?? []).length, 1); // exactly one push
});
